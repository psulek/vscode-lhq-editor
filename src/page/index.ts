...
const DragSort = require('@yaireo/dragsort').default; // Use require to import DragSort

import type { CultureInfo } from '../shared/types';
import type { IResourceElement, IResourceValueElement } from '@lhq/lhq-generators';
...
// Use get_vscode_api to fetch the acquireVsCodeApi reference
const vscode = (window as any).acquireVsCodeApi ? (window as any).acquireVsCodeApi() : undefined;


/** @type  CultureInfo[] */
const usedCultures: CultureInfo[] = [{
    engName: 'English',
    name: 'en',
    nativeName: 'English',
    lcid: 1033,
...
// Add type to getCultureName
function getCultureName(lang: string): string {
    if (lang && lang !== '') {
...
String.prototype.toPascalCase = function (): string {
    ...
    // @ts-ignore
    return _.startCase(this.toString());
};

const tooltips = new Map<string, HTMLDivElement>();

function showTooltip(uid: string, message: string, anchorEl: HTMLElement): void {
    const timeout = 3000;
...
const sampleData: IResourceElement = JSON.parse(`{
      "name": "Title",
...
createApp({
    data(): { item: IResourceElement; paramsEnabled: boolean } {
        return {
            item: sampleData as IResourceElement,
            paramsEnabled: false,
        };
    },
    ...
    computed: {
        ...
        translations(): Array<{ valueRef: IResourceValueElement; culture: string; isPrimary: boolean }> {
            const result: Array<{ valueRef: IResourceValueElement; culture: string; isPrimary: boolean }> = [];
            if (this.item && this.item.values) {
                const primaryValue = this.item.values.find((x: IResourceValueElement) => x.languageName === currentPrimaryLang);
                if (primaryValue) {
                    result.push({
                ...
                this.item.values.forEach((value: IResourceValueElement) => {
    ...
    methods: {
        ...
        editParameters(e: Event): void {
            //debugger;

            const isCancel = (e.target as HTMLElement).dataset['cancel'] === 'true';
        ...
        lockTranslation(translation: { valueRef: IResourceValueElement }): void {
    ...
    `;
        ...
        resizeAllTextAreas() {
            ...
            textareas.forEach((textarea: Element) => this.resizeTextarea(textarea as HTMLTextAreaElement));
        },
